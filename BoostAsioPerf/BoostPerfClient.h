...
inline void BoostPerfClient<SocketType>::addSocket()
{
	...
	std::lock_guard<std::mutex> lg(this->m_socketsMtx);
...
//
//template<typename SocketType>
//inline BoostPerfClient<SocketType>::BoostPerfClient(ba::io_context& ioc, std::string hostname, int port, size_t socketsToReserve) :
...
//inline void BoostPerfClient<SocketType>::initializeTempSocket()
//{
//	ba::async_connect(m_tempSocket.get()->m_socket, m_endpoint, 
//		[this](boost::system::error_code ec, const ba::ip::tcp::endpoint& endpoint)
//		{
//			if (!ec)
//			{
...
//				m_tempSocket.get()->m_socket = ba::ip::tcp::socket(m_ioc); // resets the socket
//				std::cout << "Failed to connect to: " << m_endpoint.begin()->endpoint() << '\n';
//			}
//		});
//}
...
//		bool notEnoughSockets = (m_sockets.size() < m_numOfSockets);
//		if (notEnoughSockets)
//		{
//			initializeTempSocket(); // calls async connect on the temp socket if more sockets are required
//		}
